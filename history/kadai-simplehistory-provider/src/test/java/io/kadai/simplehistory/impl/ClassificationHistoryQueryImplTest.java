...
class ClassificationHistoryQueryImplTest {
  ...
  @BeforeEach
  void setup() {
    historyQueryImpl = new ClassificationHistoryQueryImpl(kadaiHistoryEngineMock);
  }
  ...
  void should_returnList_When_CallingListMethodOnTaskHistoryQuery() throws Exception {
    ...
    doNothing().when(kadaiHistoryEngineMock).openConnection();
    doNothing().when(kadaiHistoryEngineMock).returnConnection();
