...
class SimpleHistoryServiceImplTest {
  ...
  @Mock private KadaiHistoryEngineImpl kadaiHistoryEngineMock;
  ...
  void should_VerifyMethodInvocations_When_CreateTaskHistoryEvent() throws Exception {
    ...
    verify(kadaiHistoryEngineMock, times(1)).openConnection();
    verify(taskHistoryEventMapperMock, times(1)).insert(expectedWb);
    verify(kadaiHistoryEngineMock, times(1)).returnConnection();
  ...
  void should_VerifyMethodInvocations_When_QueryTaskHistoryEvent() throws Exception {
    ...
    when(kadaiHistoryEngineMock.getConfiguration()).thenReturn(kadaiConfiguration);
    when(kadaiConfiguration.isAddAdditionalUserInfo()).thenReturn(false);

    when(kadaiHistoryEngineMock.getSqlSession()).thenReturn(sqlSessionMock);
    ...
    verify(kadaiHistoryEngineMock, times(1)).openConnection();
    verify(kadaiHistoryEngineMock, times(1)).getSqlSession();
