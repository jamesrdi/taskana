...
import io.kadai.simplehistory.impl.KadaiHistoryEngineImpl;
...
public abstract class AbstractAccTest {
  ...
  protected static KadaiHistoryEngineImpl kadaiHistoryEngine;
  ...
  protected static void initKadaiEngine(KadaiConfiguration configuration) throws SQLException {
    ...
    kadaiHistoryEngine = KadaiHistoryEngineImpl.createKadaiEngine(kadaiEngine);
  ...
  protected TaskHistoryQueryMapper getHistoryQueryMapper()
    ...
    Field sessionManagerField = KadaiHistoryEngineImpl.class.getDeclaredField("sessionManager");
    sessionManagerField.setAccessible(true);
    SqlSessionManager sqlSessionManager =
        (SqlSessionManager) sessionManagerField.get(kadaiHistoryEngine);
  ...
  protected static WorkbasketHistoryEventMapper getWorkbasketHistoryEventMapper() {
    try {
      Field sessionManager = KadaiHistoryEngineImpl.class.getDeclaredField("sessionManager");
      sessionManager.setAccessible(true);
      SqlSessionManager manager = (SqlSessionManager) sessionManager.get(kadaiHistoryEngine);
      return manager.getMapper(WorkbasketHistoryEventMapper.class);
    } catch (Exception e) {
      throw new JUnitException(
          String.format(
              "Could not extract %s from %s",
              WorkbasketHistoryEventMapper.class, KadaiHistoryEngineImpl.class));
    }
  }
  ...
  protected static ClassificationHistoryEventMapper getClassificationHistoryEventMapper() {
    try {
      ...
      throw new JUnitException(
          String.format(
              "Could not extract %s from %s",
              ClassificationHistoryEventMapper.class, KadaiHistoryEngineImpl.class));
    }
  }
