...
public interface TaskMapper {
  ...
  @Select(
      "<script>SELECT ID, EXTERNAL_ID, CREATED, CLAIMED, COMPLETED, MODIFIED, PLANNED, RECEIVED, DUE, NAME, CREATOR, DESCRIPTION, NOTE, PRIORITY, MANUAL_PRIORITY, STATE, CLASSIFICATION_CATEGORY, CLASSIFICATION_KEY, CLASSIFICATION_ID, WORKBASKET_ID, WORKBASKET_KEY, DOMAIN, BUSINESS_PROCESS_ID, PARENT_BUSINESS_PROCESS_ID, OWNER, POR_COMPANY, POR_SYSTEM, POR_INSTANCE, POR_TYPE, POR_VALUE, IS_READ, IS_TRANSFERRED, CALLBACK_INFO, CALLBACK_STATE, CUSTOM_ATTRIBUTES, "
          ...
          + "CUSTOM_INT_1, CUSTOM_INT_2, CUSTOM_INT_3, CUSTOM_INT_4, CUSTOM_INT_5, CUSTOM_INT_6, CUSTOM_INT_7, CUSTOM_INT_8, IS_LOCKED "
          + "FROM TASK "
          + "WHERE ID = #{id} "
  ...
  @Insert(
      "INSERT INTO TASK(ID, EXTERNAL_ID, CREATED, CLAIMED, COMPLETED, MODIFIED, PLANNED, RECEIVED, DUE, NAME, CREATOR, DESCRIPTION, NOTE, PRIORITY, MANUAL_PRIORITY, STATE,  CLASSIFICATION_CATEGORY, CLASSIFICATION_KEY, CLASSIFICATION_ID, WORKBASKET_ID, WORKBASKET_KEY, DOMAIN, BUSINESS_PROCESS_ID, PARENT_BUSINESS_PROCESS_ID, OWNER, POR_COMPANY, "
          ...
          + "CUSTOM_9, CUSTOM_10, CUSTOM_11,  CUSTOM_12,  CUSTOM_13,  CUSTOM_14,  CUSTOM_15,  CUSTOM_16, CUSTOM_INT_1, CUSTOM_INT_2, CUSTOM_INT_3, CUSTOM_INT_4, CUSTOM_INT_5, CUSTOM_INT_6, CUSTOM_INT_7, CUSTOM_INT_8, LOCK_EXPIRE ) "
          ...
          + "#{custom11}, #{custom12}, #{custom13}, #{custom14}, #{custom15},  #{custom16}, #{customInt1}, #{customInt2}, #{customInt3}, #{customInt4}, #{customInt5}, #{customInt6}, #{customInt7}, #{customInt8}, #{lockExpire})")
  ...
  @Update(
      "UPDATE TASK SET CLAIMED = #{claimed}, COMPLETED = #{completed}, MODIFIED = #{modified}, PLANNED = #{planned}, RECEIVED = #{received}, DUE = #{due}, NAME = #{name}, DESCRIPTION = #{description}, NOTE = #{note}, "
          ...
          + "LOCK_EXPIRE = #{lockExpire} WHERE ID = #{id}")
  ...
  @Select(
      "<script> "
          ...
          + "AND IS_LOCKED = FALSE FOR UPDATE"
          + "</when>"
          + "<otherwise>"
          + "AND IS_LOCKED = 0 FOR UPDATE"
          + "</otherwise>"
          + "</choose>"
          + "</script>")
  @Result(property = "taskId", column = "ID")
  List<String> selectWithLockFalse(@Param("taskIds") List<String> taskIds);
  ...
  @Select(
      "<script> "
          ...
          + "<choose>"
          + "<when test=\"_databaseId == 'postgres'\">"
          + "AND IS_LOCKED = TRUE FOR UPDATE"
          + "</when>"
          + "<otherwise>"
          + "AND IS_LOCKED = 1 FOR UPDATE"
          + "</otherwise>"
          + "</choose>"
          + "</script>")
  @Result(property = "taskId", column = "ID")
  List<String> selectWithLockTrue(@Param("taskIds") List<String> taskIds);

  @Update(
      "<script>"
          + "<choose>"
          + "<when test=\"_databaseId == 'postgres'\">"
          + "UPDATE TASK SET IS_LOCKED = TRUE "
          + "</when>"
          + "<otherwise>"
          + "UPDATE TASK SET IS_LOCKED = 1 "
          + "</otherwise>"
          + "WHERE ID IN (<foreach item='item' collection='taskIds' separator=',' >#{item}</foreach>)"
          + "</choose>"
          + "</script>")
  void lockTask(@Param("taskIds") List<String> taskIds);

  @Update(
      "<script>"
          + "<choose>"
          + "<when test=\"_databaseId == 'postgres'\">"
          + "UPDATE TASK SET IS_LOCKED = FALSE "
          + "</when>"
          + "<otherwise>"
          + "UPDATE TASK SET IS_LOCKED = 0 "
          + "</otherwise>"
          + "WHERE ID IN (<foreach item='item' collection='taskIds' separator=',' >#{item}</foreach>)"
          + "</choose>"
          + "</script>")
  void unlockTask(@Param("taskIds") List<String> taskIds);
}
