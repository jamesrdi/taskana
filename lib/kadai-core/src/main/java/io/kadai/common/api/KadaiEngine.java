...
public interface KadaiEngine {
  String MINIMAL_KADAI_SCHEMA_VERSION = "7.0.0";
  ...
  static KadaiEngine buildKadaiEngine(KadaiConfiguration configuration) throws SQLException {
    return buildKadaiEngine(configuration, ConnectionManagementMode.PARTICIPATE);
  }
  ...
  static KadaiEngine buildKadaiEngine(
      ...
      throws SQLException {
    return KadaiEngineImpl.createKadaiEngine(configuration, connectionManagementMode);
  }
   ...
   *     <ul>
   *       <li>{@linkplain ConnectionManagementMode#PARTICIPATE PARTICIPATE} - kadai participates
   *           in global transaction; this is the default mode
   *       <li>{@linkplain ConnectionManagementMode#AUTOCOMMIT AUTOCOMMIT} - kadai commits each
   *           API call separately
